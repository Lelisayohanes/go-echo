...
type NewRelicConfig struct {
	...
	DistributedTracingEnabled bool   `koanf:"distribution_tracing_enabled"`
...
func (c *ObservabilityConfig) Validate() error {
	...
	if !validLevels[c.Logging.Level] {
		return fmt.Errorf("invalid logging level: %s (must ve one of: debug, info, warn, error)", c.Logging.Level)
	}

	//validate slow query threshold
	if c.Logging.SlowQueryThreshold < 0 {
		return fmt.Errorf("Loggging slow_query_threshold mustbe non-negative")
	}
	return nil
}
